...
(***************************************************************************)
(* Fixed topology. Domain C has three replicas so it can transiently hold  *)
(* two Domain Leaders (one elected by a majority that excludes the other);  *)
(* domains A and B have a single replica. Global quorum is a majority of    *)
(* the three domains, so any two global quorums share at least one domain.  *)
(***************************************************************************)
Domains == {"A", "B", "C"}
Servers == {"a1", "b1", "c1", "c2", "c3"}
Nil     == "none"
...
IsGlobalQuorum(DS) ==
    /\ DS \subseteq Domains
    /\ 2 * Cardinality(DS) > Cardinality(Domains)
